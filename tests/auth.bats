...
@test "default user plain password gel" {
  local container_id
  local instance
...
@test "new user plain password gel" {
  local container_id
  local instance
...
@test "default user hashed password gel" {
  local container_id
  local instance
  ...
  run gel -I "${instance}" query "SELECT 7*3"
  [[ ${lines[-1]} = "21" ]]
}
...
@test "create role manually (via env) gel" {
  local container_id
  local instance
  ...
  run gel -I "${instance}" query "SELECT 7*4"
  [[ ${lines[-1]} = "28" ]]
}
...
@test "create role manually (via cmdline)" {
  ...
  run gel -I "${instance}" query "SELECT 7*5"
  [[ ${lines[-1]} = "35" ]]
}
...
@test "custom default branch gel" {
  local container_id
  local instance
...
@test "tls in env vars gel" {
  ...
  run gel -I "${instance}" query "SELECT 'secure'"
  [[ ${lines[-1]} = '"secure"' ]]
}
